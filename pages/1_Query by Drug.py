...
ligands_df_sub =ligands_df[['Ligand ID', 'FDA approved']]
...
int_builder = GridOptionsBuilder.from_dataframe(display_df[['Ligand ID', 'PDB_ID',  'pdb_count','Interface_ID', 'interface_count', 'FDA approved']])
...
column_names = ['PDB ID','Interface ID','Ligand ID', 'Interface Count','PDB Structure Count']
column_names = column_names + list(selected_df.columns[5:])
selected_df = selected_df.rename(columns=lambda x: x.strip())
selected_df.columns = column_names
ordered = ['Ligand ID'] + list(selected_df.columns[10:]) + ['FDA approved', 'Source Database', 'ECFP4 Members','Pharmacophore Members', 'SMILES', 'PDB ID','PDB Structure Count','Interface ID','Interface Count']
