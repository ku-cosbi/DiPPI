...
if selection == 'Filter by Ligand ID':
    ligand_selection = st.multiselect("Ligand ID", ligands_df['Ligand ID'])
    ...
    ligand_selection = st.multiselect("SMILES", ligands_df['SMILES'])
    if len(ligand_selection) > 0:
...
if len(display_df) != 0:
    ...
    if int_return["selected_rows"] != []:
        with st.spinner('Loading data...'):
            st.markdown(f'<p class="main-font">Calculated properties of the selection: </p>', unsafe_allow_html=True)
        ...
        st.markdown(f'<p class="main-font"> Select from the calculated property table to download: </p>', unsafe_allow_html=True)
